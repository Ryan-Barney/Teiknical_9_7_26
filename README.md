...
1. Install dependencies - installs python packages specified in requirements.txt
make setup
...
load_data.py rebuilds Loblaw_Bio_immune_trial.db from cell-count.csv. bob_analysis.py then queries the database and generates the outputs Bob wants.
...
part4_samples.csv — Day 0 melanoma PBMC samples from miraclib-treated patients.
part4_summary.csv — summaries by project, treatment response, and sex.

3. Start the interactive dashboard
make dashboard

The dashboard runs a local HTTP server on port 8000. In GitHub Codespaces, open the forwarded port when prompted or navigate to the Ports tab and open port 8000 in the browser.
...
This design has several advantages for scaling. With hundreds of projects and thousands or more samples, repeated subject and sample metadata do not need to be duplicated across every cell measurement. New cell populations can also be incorporated without altering the table, and they can simply be added as new cell_type records. Primary and foreign keys enforce the relationships between subjects, samples, and measurements and help preserve data integrity.
...
The percentage was rounded to the hundreds decimal place.
...
The matching sample-level records are also exported for inspection if Bob needs.

Dashboard

The interactive dashboard is available here after running:
make dashboard
